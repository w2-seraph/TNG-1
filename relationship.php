...
if(is_numeric($primarypersonID)) $primarypersonID = $tngconfig['personprefix'] . $primarypersonID . $tngconfig['personsuffix'];
...
function finishRelationship($couple) {
	global $pedigree, $totalRelationships, $needmore, $gens, $maxrels, $session_norels, $saveindent;
...
function checkpersonup( $nextcouple ) {
	...
	if($needmore) {
		...
		if($gens->upptr < count($gens->uplist[$lastup])) {
			$nextcouple = isset($gens->uplist[$lastup][$gens->upptr]) ? $gens->uplist[$lastup][$gens->upptr] : "";
			$gens->upcount--;
