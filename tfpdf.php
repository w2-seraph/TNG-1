...
function __construct($orientation='P', $unit='mm', $size='A4')
{
	...
	// Page sizes
	$this->StdPageSizes = array('a2'=>array(1190.55,1683.78), 'a3'=>array(841.89,1190.55), 'a4'=>array(595.28,841.89), 'a5'=>array(420.94,595.28),
		'letter'=>array(612,792), 'legal'=>array(612,1008), 'ledger'=>array(1224,792), 'tabloid'=>array(792,1224));
...
function AddFont($family, $style='', $file='', $uni=false)
{
	...
	if($file=='') {
	   if ($uni) {
			$file = str_replace(' ','',$ucFamily).'/'.str_replace(' ','',$ucFamily).$style.'.ttf';
		}
		else {
			$file = str_replace(' ','',$family).'/'.str_replace(' ','',$family).strtolower($style).'.php';
	   }
	}
	...
	if ($uni) {
		...
		$unifilename = $this->fontpath.'unifont/'.$ucFamily.'/'.$family.strtolower($style);
		$name = '';
		$originalsize = 0;
		...
		if (!isset($type) ||  !isset($name) || $originalsize != $ttfstat['size']) {
			...
			if (is_writable(dirname($this->fontpath.'unifont/'.'x'))) {
				...
				fclose($fh);
				if (file_exists($unifilename.'.cw127.php')) {
...
function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false, $hangIndent=0)
{
